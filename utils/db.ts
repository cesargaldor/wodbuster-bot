...
export function getSavedCookie() {
  try {
    ...
    if (fs.existsSync(COOKIE_FILE)) {
      ...
      console.log("✅ Cookie encontrada:", cookie);
...
export async function loginAndSaveCookie() {
  console.log("🌐 Session expired or missing. Launching Playwright...");
  ...
  try {
    ...
    try {
      const labelSelector = 'label[for="body_body_CtlConfiar_CtlNoSeguro"]';
