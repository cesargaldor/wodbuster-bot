...
import { getSavedCookie, getDaysConfig } from "./utils/db";
...
const TARGET_DAY_ENV = process.env.TARGET_DAY as string;
...
async function runSniper() {
  try {
    // 0. COMPROBACIÓN DE SESIÓN Y CONFIGURACIÓN
    await checkSession();
    const cookieValue = getSavedCookie();
    ...
    // 1. GESTIÓN DE FECHAS
    if (TARGET_DAY_ENV) {
      targetDate = DateTime.fromFormat(TARGET_DAY_ENV, "dd/MM/yyyy", { zone: ZONE }).startOf("day");
    } else {
    ...
    // 2. FILTRO DE DÍAS ACTIVOS (Telegram)
    const targetWeekday = targetDate.weekday;

    if (!activeDays.includes(targetWeekday)) {
      console.log(`ℹ️ El objetivo ${targetDate.toFormat("dd/MM")} no está activado en Telegram. Cancelamos...`);
      return;
    }

    // 3. LÓGICA DE VARIABLES SEGÚN EL DÍA
    const isFriday = targetWeekday === 5;
    const isSaturday = targetWeekday === 6;
    let currentTargetHour = TARGET_HOUR_NORMAL;
    let currentTargetType = TARGET_TYPE;

    if (isFriday) {
      console.log("🔥 Objetivo detectado: VIERNES. Cambiando hora...");
      currentTargetHour = "19:45:00";
    }

    if (isSaturday) {
      console.log("🔥 Objetivo detectado: SÁBADO. Cambiando hora...");
      currentTargetHour = TARGET_HOUR_SATURDAY;
      currentTargetType = TARGET_TYPE_SATURDAY;
    }
    ...
    // 5. BUSCAR ID DE CLASE
    const classId =
      resp.data.ListClases?.find((c: any) => c.Hora === currentTargetHour && c.NombreE.includes(currentTargetType))?.Id ??
      resp.data.Data?.find((d: any) => d.Hora === currentTargetHour)?.Valores.find((v: any) =>
        v.Valor.Nombre.includes(currentTargetType),
      )?.Valor.Id;

    if (!classId)
      throw new Error(`No se encontró la clase ${currentTargetType} del ${targetDate.toISODate()} a las ${currentTargetHour}`);

    console.log(`📍 ID de clase localizado: ${classId}`);

    // 6. ESPERA AL DISPARO (22:00:00)
    if (!TARGET_DAY_ENV) {
